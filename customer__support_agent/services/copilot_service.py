...
from customer__support_agent.integrations.memory.mem0_store import (
    CustomerMemoryStore,
)
...
class SupportCopilot:
    def __init__(self,settings: Settings):
        ...
        try:
            self.memory = CustomerMemoryStore(settings=self._settings,llm=self._llm)
    ...
    def list_customer_memories(
        ...
        for scope_user_id in scope_user_ids:
            ...
            raw_hits.extend(self._annotate_memory_scope(hits,scope_user_id))
        return self._dedupe_memory_hits(raw_hits, limit=limit)
    ...
    def _format_kb(kb_hits: list[dict[str,Any]]) -> str:
        if not kb_hits:
            return "- No revelant knowledge-base chunks found "
    ...
    def _extract_agent_draft_and_tool_calls(
        self, agent_results: Any
    ) -> tuple[str,list[dict[str,Any]]]:
        raw_messages: Any
        if isinstance(agent_results, dict):
            raw_messages = agent_results.get("messages") or []
        else:
            raw_messages = getattr(agent_results,"messages",[]) or []
    ...
    def _deterministic_fallback(
        ...
        action_line = (
            ...
            else "Our support team is reviewing your account and issues details now."
        )
