...
from customer__support_agent.schemas.api import DraftResponse, DraftUpdateRequest
from customer__support_agent.services.draft_service import DraftService
...
def get_draft_route(
    ...
    draft = drafts_repo.get_latest_for_ticket(ticket_id=ticket_id)
    if not draft:
        raise HTTPException(status_code=404, detail="Draft not found")
    return drafts_service.serialize_draft(draft)
...
def update_draft_route(
    ...
    existing = drafts_repo.get_by_id(draft_id=draft_id)
    if not existing:
        raise HTTPException(status_code=404, detail="Draft not found")
    ...
    if payload.status == "accepted":
        relation = drafts_repo.get_ticket_and_customer_by_draft(draft_id=draft_id)
        if relation:
